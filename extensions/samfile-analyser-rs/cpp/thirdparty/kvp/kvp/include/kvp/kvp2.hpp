...
namespace KeyValueParser2
{
    ...
    class KeyValueStore
    {
      ...
      public:
        ...
        const StoreType &get_data() const { return data; }
    };

    // --- Parser functions ---
    template <typename StoreType = std::unordered_map<std::string, std::string>>
    KeyValueParser2::KeyValueStore<StoreType>
    parse_kvp2(const std::string &input)
    {
        ...
        while (std::getline(ss, line))
        {
            ...
            std::string_view key =
                KeyValueParser::trim_view(line_view.substr(0, pos));
            std::string_view value =
                KeyValueParser::trim_view(line_view.substr(pos + 1));
    ...
    // the 2nd Map will overright the first one when entries are already
    // existing
    template <typename StoreType = std::unordered_map<std::string, std::string>>
    ...
    template <typename StoreType = std::unordered_map<std::string, std::string>>
    void append_entry(StoreType &store, const std::string &key,
    ...
    template <typename StoreType = std::unordered_map<std::string, std::string>>
    void overwrite_entry(StoreType &store, const std::string &key,
                         const std::string &value)
    {
        if (store.contains(key))
        {
    ...
    inline std::string append_entry(const std::string &key,
                                    const std::string &value,
    ...
    inline std::string overwrite_entry(const std::string &key,
                                       const std::string &value,
                                       const std::string &input)
    {
        std::stringstream ss(input);
    ...
    inline std::string delete_entry(const std::string &key,
                                    const std::string &input)
    {
        std::stringstream ss(input);
        std::string line;
