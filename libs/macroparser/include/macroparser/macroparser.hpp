...
namespace StringUtils
{
    ...
    // Pr�ft, ob String mit anderem String beginnt
    ...
    // Pr�ft, ob String an Position pos einen String enth�lt
...
namespace MacroParserUtils
{
    ...
    struct Macro
    {
        ...
        // z.B. "#define LOG(fmt, ...) ..."). Zus�tzliche Aufrufargumente
        // jenseits der benannten Parameter werden dann �ber den speziellen
        // Bezeichner __VA_ARGS__ im Body verf�gbar gemacht.
        bool variadic = false;
...
class MacroParser
{
  private:
    ...
    // Wie expandText, aber speziell f�r #if-Ausdr�cke: das Argument
    // von "defined(NAME)" / "defined NAME" wird NICHT expandiert,
    // sonst w�rde z.B. "#if defined(FOO)" kaputtgehen sobald FOO
    // ein Objekt-Makro ist (FOO w�rde textuell durch seinen Wert
    // ersetzt, bevor "defined" �berhaupt ausgewertet wird).
    ...
    // funktions�hnlichen Makros 'name' (inkl. Klammer-Argumentliste,
    ...
    // 'params' enth�lt die Parameternamen (inkl. ggf. "__VA_ARGS__"
    ...
    // reduziert, f�hrendes/folgendes Whitespace entfernt, '"' und
    // '\' werden escaped).
